...
namespace UnityRoyale
{
    public class CardManager : MonoBehaviour
    {
        ...
        private GameObject activeSplashAnimation;
        [SerializeField] private GameObject uiHolder;
        [SerializeField] private GameObject firstHolder;

        private void Awake()
        {
            previewHolder = new GameObject("PreviewHolder");
            cards = new Card[3]; //3 is the length of the dashboard
            StartCoroutine(ShowUIObjectForDuration(13f));
        }
        ...
        */
        ...
        private void CardDragged(int cardId, Vector2 dragAmount)
        {
            ...
            if (planeHit)
            {
                if (!cardIsActive)
                {
                    ...
                    // Spawn all the preview Placeables and parent them to the previewHolder
                    for (int i = 0; i < dataToSpawn.Length; i++)
                    {
                        GameObject prefabToSpawn = (dataToSpawn[i].alternatePrefab == null) ? dataToSpawn[i].associatedPrefab : dataToSpawn[i].alternatePrefab;

                        GameObject newPlaceable = Instantiate<GameObject>(prefabToSpawn,
                            hit.point + offsets[i] + inputCreationOffset,
                            Quaternion.Euler(90f, 0f, 0f), // Desired rotation (90, 0, 0)
                            previewHolder.transform);

                        // Retrieve the PlaceableComponent from the instantiated object
                        PlaceableComponent placeableComponent = newPlaceable.GetComponent<PlaceableComponent>();
                        if (placeableComponent != null)
                        {
                            // Set the PlaceableData reference in the PlaceableComponent
                            placeableComponent.placeableData = dataToSpawn[i];
                        }

                        // Apply rotation to the instantiated object
                        newPlaceable.transform.rotation = Quaternion.Euler(90f, 0f, 0f);
                    }
        ...
        private IEnumerator DestroySplashAnimation(GameObject splashAnimation, float delay)
        {
            yield return new WaitForSeconds(delay);

            if (splashAnimation != null)
            {
                Destroy(splashAnimation);
                activeSplashAnimation = null;
            }
        }

        private IEnumerator ShowUIObjectForDuration(float duration)
        {
            ...
            firstHolder.SetActive(false);
        }        
        private IEnumerator ShowHolderObjectForDuration(float duration)
        {
            // Show the UI GameObject
        ...
        private void CardReleased(int cardId)
        {
            ...
            if (Physics.Raycast(ray, out hit, Mathf.Infinity, playingFieldMask))
            {
                ...
                // Instantiate the splashAnimation object from CardData if it exists
                GameObject splashAnimationPrefab = cards[cardId].cardData.splashAnimation;
                if (splashAnimationPrefab != null && activeSplashAnimation == null)
                {
                    Vector3 splashAnimationPosition = new Vector3(0f, 30.24f, -8.43f);
                    Quaternion splashAnimationRotation = Quaternion.Euler(90f, 0f, 0f); // Desired rotation (90, 0, 0)
                    activeSplashAnimation = Instantiate<GameObject>(splashAnimationPrefab, splashAnimationPosition, splashAnimationRotation);
                    StartCoroutine(DestroySplashAnimation(activeSplashAnimation, 0.7f));
                    StartCoroutine(ShowHolderObjectForDuration(5f));
                }
            }
            else
            {
